...
static void Update(uint32_t Row, uint32_t Col)
{
    ...
    SCREEN.seekp(((Row + 1) * frw) + (Row * fdw) + (((2 * Col) + 1) * fd), std::ios::beg).write(STRONG_GREEN.data(), STRONG_GREEN.size());
}
...
COLS = (2 * WIDTH + 1) * CELL_WIDTH
LINES = (2 * HEIGHT + 1) * CELL_HEIGHT

//TODO: Update optimization. After update was called print only part that has changed.
//TODO: Fix when rectangular size of window.
